...
class VehicleRecordPostProcessor:
    ...
    def post_process_vissim_data(self, vehicle_records):
        ...
        warm_up_time = 60
        self.remove_early_samples(vehicle_records, warm_up_time)
        ...
        self.compute_values_relative_to_leader(vehicle_records)
    ...
    def post_process_ngsim_data(self, vehicle_records):
        ...
        warm_up_time = min(vehicle_records.loc[vehicle_records['leader_id'] > 0,
                                               'time'])
        self.remove_early_samples(warm_up_time, vehicle_records)
        # By convention, if vehicle has no leader, we set it as its own leader
    ...
    def post_process_synthetic_data(self, vehicle_records):
        """
    ...
    @staticmethod
    def compute_delta_v_old(vehicle_records):
        print('WARNING: deprecated method [June 2, 2021]')
        # vehicle_records = self.veh_records
        print('Computing delta v for {} samples'.
              format(vehicle_records.shape[0]))

        vehicle_records.set_index('time', inplace=True)
        max_size = max(vehicle_records['veh_id']) + 1  # +1 due to zero indexing
        identity = np.eye(max_size)
        sim_times = np.unique(vehicle_records.index)
        percent = 0.1
        out_of_bounds_counter = 0
        for i in range(len(sim_times)):
            current_time = sim_times[i]
            current_data = vehicle_records.loc[[current_time]]
            ...
            vehicle_records.loc[current_time, 'delta_v'] = delta_v
            vehicle_records.loc[current_time, 'leader_type'] = leader_type
        ...
        print('Out of bounds leaders found in {} out of {} samples ({}%)'.
              format(out_of_bounds_counter, vehicle_records.shape[0],
                     int(out_of_bounds_counter / vehicle_records.shape[
                         0] * 100)))

        vehicle_records.reset_index(inplace=True)

    def compute_values_relative_to_leader(self, vehicle_records: pd.DataFrame):
