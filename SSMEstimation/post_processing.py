...
def post_process_data(data_source: str,
                      vehicle_records: pd.DataFrame) -> None:
...
def post_process_vissim_vehicle_record(vehicle_records) -> None:
    """
...
def post_process_ngsim_data(vehicle_records) -> None:
    """
...
def post_process_synthetic_data(vehicle_records) -> None:
    """
...
def create_summary_with_risks(
        ...
        analyze_lane_change: bool = True, debugging: bool = False) -> None:
    """
    Reads multiple vehicle record data files, postprocesses them,
    computes and aggregates SSMs results, extracts risky maneuvers, and
    find traffic light violations. SSMs, risky maneuvers and
...
def find_lane_change_issues(
        network_name: str, scenario_info: ScenarioInfo) -> None:
    """
...
def create_time_in_minutes_from_intervals(data: pd.DataFrame) -> None:
    ...
    create_time_from_intervals(data)
    data["time"] = data["time"] / seconds_in_minute


def create_time_from_intervals(data: pd.DataFrame) -> None:
    """ Creates a "time" column in seconds
...
def find_removed_vehicles(network_name: str,
    ...
    for (vehicle_records, _) in data_generator:
        ...
        n_removed_vehicles[-1].append(len(removed_vehs))
        print("  {} removed vehs".format(n_removed_vehicles[-1]))
    print("Mean # of removed vehicles: {}".format(np.mean(n_removed_vehicles)))
...
def add_main_ssms(vehicle_records: pd.DataFrame, ssm_names: list[str]) -> None:
...
def create_time_bins_and_labels(period, vehicle_records) -> None:
...
def compute_distance_to_leader(data_source: str, veh_data: pd.DataFrame,
                               ...
                               adjusted_leader_idx: np.array) -> None:
    """
...
def compute_gap_between_vehicles(
        leader_rear_x: np.ndarray, leader_rear_y: np.ndarray,
        follower_front_x: np.ndarray, follower_front_y: np.ndarray
) -> np.ndarray:
...
def drop_warmup_samples(data: pd.DataFrame, warmup_time: float,
                        normalize_time: bool = False) -> None:
    """
...
def drop_late_samples(data: pd.DataFrame, last_time: float) -> None:
    """
    Drops samples with time above last time.
    :param data: Any VISSIM output or post processed data
    :param last_time: Time *in minutes*
    """
    if last_time is not None:
        last_time *= 60
        if "time" not in data.columns:
            create_time_from_intervals(data)
        data.drop(index=data[data["time"] > last_time].index, inplace=True)
...
def remove_false_lane_change_starts(veh_data: pd.DataFrame,
                                    lane_change_data: pd.DataFrame) -> None:
    """
...
def get_absolute_lane(lane: pd.Series, link: pd.Series) -> pd.Series:
    """
...
def get_lane_change_crossing_and_end_times(
        single_veh_data, lc_data: pd.Series) -> tuple[float, float]:
    """
...
def compute_initial_lane_change_risks(lc_data: pd.Series
...
def get_risks_during_lane_change(
        ...
        lc_data: pd.Series, end_time: float) -> tuple[float, float, float]:
    """
...
def get_total_lane_change_risk(
        ...
        end_time: float) -> tuple[float, float, float]:
    """
...
def label_lane_changes(scenario_name: str, veh_data: pd.DataFrame,
    ...
    right_mandatory_mask = ((lc_direction < 0)
                            & took_off_ramp_mask)
    lc_data.loc[left_mandatory_mask | right_mandatory_mask, "mandatory"] = True
...
def compute_collision_free_gap(
        follower: Vehicle, leader: Vehicle, is_lane_changing: bool,
        follower_vel: Union[float, list[float]],
        leader_vel: Union[float, list[float]]) -> Union[float, list[float]]:
    """
...
def compute_gap_thresholds_for_collision(
        follower: Vehicle, leader: Vehicle,
        leader_vel: Union[float, np.ndarray],
        delta_vel: Union[float, np.ndarray],
        follower_max_brake: Union[float, np.ndarray],
        follower_lambda1: Union[float, np.ndarray],
        follower_tau_j: Union[float, np.ndarray]
...
def compute_risk_given_phase(
        follower: Vehicle, leader: Vehicle,
        follower_vel: Union[float, np.ndarray],
        leader_vel: Union[float, np.ndarray],
        delta_vel: Union[float, np.ndarray], gap: Union[float, np.ndarray],
        follower_max_brake: Union[float, np.ndarray],
        follower_lambda1: Union[float, np.ndarray],
        case: Union[int, np.ndarray], print_warning: bool = True
...
def identify_ngsim_lane_changes(data: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    data.loc[(data["lane"] == 7)
             & (data["future_lane"] == 6), "future_lane"] = 7
    data.loc[(data["lane"] == 6)
             & (data["future_lane"] == 8), "future_lane"] = 6
...
def find_ngsim_lane_change_start(data: pd.DataFrame,
                                 lane_change_idx: pd.Index) -> pd.DataFrame:
    """
...
class SSMEstimator:
    ...
    def __init__(self, veh_data: pd.DataFrame):
        self.veh_data: pd.DataFrame = veh_data

    def include_ssms(self, ssm_names: list[str]) -> None:
        for ssm in ssm_names:
            self.include_ssm_by_name(ssm)

    def include_ssm_by_name(self, ssm_name: str) -> None:
    ...
    def include_ttc(self, safe_threshold: float = ttc_threshold) -> None:
        """
    ...
    def include_drac(self, safe_threshold: float = drac_threshold) -> None:
        """
    ...
    def include_cpi(self, max_decel_data, is_default_vissim=True) -> None:
        """
    ...
    def include_collision_free_gap(self, same_type_gamma=1,
                                   consider_lane_change: bool = True) -> None:
        """
    ...
    def include_estimated_risk(self, same_type_gamma: float = 1,
                               rho: float = 0.2,
                               free_flow_velocity=None) -> None:
        """
    ...
    def include_barrier_function_risk(self) -> None:
        """
    ...
    def _compute_collision_free_gap(
            self, veh_idx: pd.Series, follower: Vehicle, leader: Vehicle,
            consider_lane_change: bool = False) -> np.ndarray:
        """
    ...
    def _compute_risk(self, veh_idx: pd.Series,
                      follower: Vehicle, leader: Vehicle,
                      consider_lane_change: bool = True) -> np.ndarray:
    ...
    @staticmethod
    def compute_risk_during_jerk_phase(
            follower: Vehicle, leader: Vehicle, gap: np.ndarray,
            follower_vel: np.ndarray, leader_vel: np.ndarray,
            leader_at_full_stop: bool, print_warning: bool = True
    ) -> np.ndarray:
    ...
    @staticmethod
    def get_braking_parameters_over_time(
            vehicle_data: pd.DataFrame, vehicle: Vehicle,
            consider_lane_change: bool = True
...
class SSMProcessor(VISSIMDataPostProcessor):
    ...
    def set_measurements(self, ssm_names: list[str]) -> None:
        """
...
class RiskyManeuverProcessor(VISSIMDataPostProcessor):
    ...
    def set_risk(self, risk_name: str) -> None:
        """
...
class PlatoonLaneChangeProcessor(VISSIMDataPostProcessor):
    ...
    def post_process(self, data) -> pd.DataFrame:
        ...
        result_df["platoon_id"] = platoon_ids["platoon_id"]["first"]
...
class PlatoonLaneChangeImpactsProcessor(VISSIMDataPostProcessor):
    ...
    def accel_cost(self, accel) -> float:
...
def compute_accel_cost(platoon_vehicles: pd.DataFrame) -> None:
    ...
    def help_filter(data):
        return data.loc[(data["time"] >= data["maneuver_start_time"])
                        & (data["time"] <= data["maneuver_end_time"]), "ax"]
...
def create_summary_for_single_scenario(
        ...
        debugging: bool = False) -> None:
    """
    Reads all vehicle record data files from the given scenario, postprocesses
    them,computes and aggregates SSMs results, extracts risky maneuvers, and
    find traffic light violations. SSMs, risky maneuvers and
    violations are saved as csv files per vehicle input.
...
def is_value_unique_in_column(s: pd.Series) -> bool:
    a = s.to_numpy()
    return (a[0] == a).all()
