...
class VissimDataReader(DataReader):
    ...
    controlled_vehicle_types = [VehicleType.LONGITUDINAL_CONTROL,
    ...
    def load_data_with_controlled_vehicles_percentage(
        ...
        for i in range(1, len(percentage)):
            new_data = self.load_data(
                self.find_highest_file_number(percentage[i]),
                percentage[i])
            ...
            try:
                if sim_output.empty:
                    ...
                    sim_output = sim_output.append(
                        self.load_data(i, controlled_vehicles_percentage))
            except ValueError:
    ...
    def find_highest_file_number(self, percentage: [int, str]) -> \
            int:
    ...
    def _match_sim_number_to_vehicle_input(self, data):
        ...
        input_per_lane = veh_inputs.loc[selected_idx, 'input_per_lane']
...
class VehicleInputReader(VissimDataReader):
    ...
    _file_format = '.att'  # TODO
    _data_identifier = '_'  # TODO
    _header_identifier = ''  # TODO
    _header_map = {  # TODO
        'SIMRUN': 'simulation_number', 'TIMEINT': 'time_interval',
        'LINKEVALSEGMENT': 'link_segment_number', 'DENSITY': 'density',
        'DELAYREL': 'delay_relative', 'SPEED': 'average_speed',
    ...
    def load_data_with_controlled_vehicles_percentage(
        ...
        last_file_number = self.find_highest_file_number(percentage)
        # TODO Map file number to random seed
        # Hard to know how to proceed without any idea of what comes in the file
...
class SSMDataReader(VissimDataReader):
    ...
    def load_data(self, file_identifier: int = None,
        ...
        sim_output.rename(columns={'exact_risk_no_lane_change':
                                   'risk_no_lane_change'}, inplace=True)
