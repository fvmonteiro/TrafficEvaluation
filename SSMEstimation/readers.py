...
class VissimDataReader(DataReader):
    ...
    def find_min_max_file_number(self, percentage: Union[int, str],
        ...
        vehicle_input_folder = (file_handling.
...
class PostProcessedDataReader(DataReader):
    ...
    def __init__(self, data_source, file_name: str = None):
        ...
        if (data_source.lower() == 'vissim'
                and file_name in file_handling.network_names_map):
            file_name = file_handling.network_names_map[file_name]
...
class TrafficLightSourceReader:
    _file_extension = '.csv'
    _vissim_networks_folder = file_handling.get_networks_folder()
    _data_identifier = '_source_times'

    def __init__(self, network_name: str):
        file = (file_handling.get_file_name_from_network_name(network_name)
                + self._data_identifier + self._file_extension)
        self._file_address = os.path.join(self._vissim_networks_folder,
                                          file)

    def load_data(self) -> pd.DataFrame:
        return pd.read_csv(self._file_address)
...
class SignalControllerFileReader(DataReader):
    ...
    def __init__(self, network_name):
        network_file = file_handling.get_file_name_from_network_name(
            network_name)
        # For now, the network_name traffic_lights yields a network file
        # traffic_lighs_study/traffic_lights_study. So we must get only the
        # directory
        dir_name, file_name = os.path.split(network_file)
        DataReader.__init__(self, os.path.join(self._vissim_networks_folder,
                                               dir_name),
                            file_name)
    ...
    def load_data(self, file_identifier: int) -> ET.ElementTree:
        ...
        :return:
        """
        full_address = os.path.join(self.data_dir, self.network_name
                                    + str(file_identifier)
                                    + self._file_extension)
        try:
            with open(full_address, 'r') as file:
